...
- 🌓 **테마 전환**: 라이트/다크 모드 지원
- 💾 **로컬 우선 저장**: Electron 파일 시스템 또는 브라우저 localStorage 사용
- ☁️ **클라우드 동기화 (옵션)**: Firebase Firestore로 기기간 동기화 가능
...
- **데이터 저장**:
  - Electron: 로컬 파일 시스템 (JSON)
  - 브라우저: localStorage
  - 클라우드 (옵션): Firebase Firestore
...
PrimeRing은 **Firebase 설정 없이도 바로 사용 가능**합니다!

#### 옵션 1: 로컬 저장소만 사용 (기본, 추천)

별도 설정 없이 바로 사용 가능합니다:
- **Electron 앱**: 로컬 파일 시스템에 JSON 형식으로 저장
- **브라우저 모드**: localStorage에 저장

데이터는 기기 내에 안전하게 보관되며 외부로 전송되지 않습니다.

#### 옵션 2: Firebase 클라우드 동기화 (선택사항)

여러 기기에서 데이터를 동기화하려면 Firebase를 설정할 수 있습니다:

1. **`.env.local` 파일 생성**

루트 디렉토리의 `.env.local` 파일을 참고하여 정보를 입력합니다.
...
`.env.local` 파일에 Firebase 구성 정보를 입력합니다:
...
│   │   ├── aiService.ts      # MLC WebLLM (로컬 AI)
│   │   ├── diaryService.ts   # 다이어리 CRUD (로컬 저장소)
│   │   └── eventService.ts   # 이벤트 CRUD (로컬 저장소)
│   │
...
### 1. Firebase 관련 경고 (무시 가능)

**증상**: 콘솔에 `ℹ️ Firebase not configured. Using local storage only.` 메시지

**설명**: 이는 정상입니다! Firebase 없이도 앱은 완벽하게 작동합니다. 로컬 저장소를 사용합니다.

**Firebase를 사용하려면**:
- `.env.local` 파일 생성 및 Firebase 구성 정보 입력
